...
export default function AdminCakesPage() {
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      setMenuItems(data.filter(item => item.category.toLowerCase() === "cakes"))
    } catch (err) {
  ...
  const handleAddEditMenuItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(error.message || "There was an error saving the cake. Please try again.")
    } finally {
      setIsSubmitting(false)
      setUploadingImage(false)
    }
  ...
  const handleDeleteMenuItem = async (id: number) => {
    ...
    try {
      ...
      setError("There was an error deleting the cake. Please try again.")
    } finally {
      setDeletingId(null)
    }
  }
          ...
          <DialogContent className="sm:max-w-[500px] bg-white border-gold text-royal-blue">
            ...
            <form onSubmit={handleAddEditMenuItem} className="grid gap-4 py-4">
              ...
              <DialogFooter>
                ...
                >
                  {isSubmitting || uploadingImage ? "Saving..." : (currentMenuItem ? "Save Changes" : "Add Cake")}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
