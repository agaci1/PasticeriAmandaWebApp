...
export default function AdminSweetsPage() {
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      setMenuItems(data.filter(item => item.category.toLowerCase() === "sweets"))
    } catch (err) {
  ...
  const handleAddEditMenuItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(error.message || "There was an error saving the sweet. Please try again.")
    } finally {
      setIsSubmitting(false)
      setUploadingImage(false)
    }
  ...
  const handleDeleteMenuItem = async (id: number) => {
    ...
    try {
      ...
      setError("There was an error deleting the sweet. Please try again.")
    }
  }
          ...
          <DialogContent className="sm:max-w-[500px] bg-white border-gold text-royal-blue">
            ...
            <form onSubmit={handleAddEditMenuItem} className="grid gap-4 py-4">
              ...
              <DialogFooter>
                ...
                >
                  {isSubmitting || uploadingImage ? "Saving..." : (currentMenuItem ? "Save Changes" : "Add Sweet")}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
